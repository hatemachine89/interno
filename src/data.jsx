
export const MenuData = [
    { MenuItem:"Home",
      Link:"/" 
    },
    { MenuItem:"About Us ",
      Link:"/about-us"  
    },
    { MenuItem:"Services",
      Link:"/services"  
    },
    { MenuItem:"Pricing",
      Link:"/pricing"  
    },
    { MenuItem:"Projects",
      Link:"/projects"  
    },
    { MenuItem:"Contact",
      Link:"/contact"  
    },
  
]

export const testimonials =[

    {
        clientName:"Nattasha Mith",
        location:"Sydney, USA",
        review:"Lorem Ipsum is simply dummy text of the typesetting industry. Ipsum has been.",
        img:require("./assets/testimonial1.png")
    },
    {
        clientName:"Raymond Galario",
        location:"Sydney, Australia",
        review:"Lorem Ipsum is simply dummy text of the typesetting industry. Ipsum has been.",
        img:require("./assets/testimonial3.png")
    },
    {
        clientName:"Benny Roll",
        location:"Sydney, New York",
        review:"Lorem Ipsum is simply dummy text of the typesetting industry. Ipsum has been scrambled.",
        img:require("./assets/testimonial3.png")

    },


]

export const Images =[
   
            {image:require("./assets/01.png")},
            {image:require("./assets/02.png")},
            {image:require("./assets/03.png")},
            {image:require("./assets/04.png")},
            {image:require("./assets/05.png")},


]

export const projectData=  [
{
    projectTitle:"Modern Kitchen",
    projectTag:"Decor / Artchitectur",
    projectImg:require("./assets/projectCard1.png")
},
{
    projectTitle:"Modern Kitchen2",
    projectTag:"Decor / Artchitectur",
    projectImg:require("./assets/projectCard4.png")
},
{
    projectTitle:"Modern Kitchen3",
    projectTag:"Decor / Artchitectur",
    projectImg:require("./assets/projectCard2.png")
},
{
    projectTitle:"Modern Kitchen4",
    projectTag:"Decor / Artchitectur",
    projectImg:require("./assets/projectCard3.png")
}

]

export const blogData = [
    {
        title:"Let’s Get Solution For Building Construction Work",
        tag:"Kitchen Design",
        date:"26 December, 2023",
        blogImg:require("./assets/blogimg1.png")

    },
    {
        title:"Low Cost Latest Invented Interior DesigningyIdeas.",
        tag:"Living Design",
        date:"24 December, 2023",
        blogImg:require("./assets/blogimg2.png")
    }, 
    {
        title:"Let’s Get Solution For Building Construction Work",
        tag:"Bathroom Design",
        date:"22 December, 2023",
        blogImg:require("./assets/blogimg3.png")
    },

]


export const aboutData = [

{
    title: "What We Do",
    description:"It is a long established fact that a reader will be distracted by the of readable content of a page when lookings at its layouts the points of using that it has a more-or-less normal",
    buttonText:"Our Concepts",
    columnPic:require("./assets/whatwedoabout.jpg"),
    reverseColumn:false

},
{
    title: "The End Result",
    description:"It is a long established fact that a reader will be distracted by the of readable content of a page when lookings at its layouts the points of using that it has a more-or-less normal",
    buttonText:"Our Portfolio",
    columnPic:require("./assets/aboutcolimage2.png"),
    reverseColumn:true
}


]
